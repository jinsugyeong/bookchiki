...
3. **실시간 웹 검색 (Tavily):** 질문 키워드를 기반으로 인터넷에서 실존하는 도서 후보를 검색하여 '실시간 검색 컨텍스트' 구성 (할루시네이션 방지 및 최신 데이터 반영)
4. LLM에 다음을 컨텍스트로 주입:
   - 취향 프로필: `preference_summary`, `preferred_genres`, `disliked_genres`, `top_rated_books`
   - RAG 검색 결과 (내부 지식 베이스)
   - Tavily 검색 결과 (외부 실존 도서 목록)
5. **LLM 선별:** 컨텍스트를 종합하여 약 10권의 후보 도서 선별
6. **알라딘 엄격 검증 (Strict Validation):** 후보 도서들을 알라딘 API에서 순차적으로 검색하여 **100% 실존하는 책만** 결과에 포함
7. **결과 보장:** 검증된 실존 도서 3권을 꽉 채울 때까지 루프 수행 및 최종 결과 반환 (캐시 overwrite 없음)

**주의:**
- 환경 변수에 `TAVILY_API_KEY`가 설정되어 있어야 실시간 웹 검색이 작동합니다.
- 캐시를 수정하지 않으므로 여러 번 질문 가능하며, 추천 결과에서 "읽고 싶어요" 버튼 클릭 시 서재에 추가됩니다.
