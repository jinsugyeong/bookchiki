...
### Docker Compose로 백엔드 + postgres + opensearch 실행
...
**주의:** 현재 이 프로젝트는 docker 실행시 마이그레이션이 자동으로 진행됩니다.
---
...
**토큰 관리:**
- **Access Token:** 기본 만료 15분 (환경변수로 조정 가능)
...
│
├── docker-compose.yml        # 로컬 개발용 (백엔드 + PostgreSQL + OpenSearch)
├── .gitignore
