...
- **시스템 2 (자연어 질문 기반):**
  - **고도화 완료** (`/recommendations/ask`): 취향 프로필 + RAG + **Tavily Web Search** → LLM 선별 → **알라딘 엄격 검증(Strict Validation)** 파이프라인
  - 할루시네이션 방지를 위해 실시간 웹 검색 후보군(10권)을 먼저 확보하고 알라딘 API로 실재 여부를 대조하여 최종 3권 보장
