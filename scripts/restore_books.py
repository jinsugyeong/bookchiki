"""
books OpenSearch 인덱스 복원 (재임베딩 없이).

backup_books.py로 생성한 JSON 덤프 파일을 읽어
...
def _ensure_books_index(os_client, clear: bool) -> None:
    """books 인덱스 준비. clear=True이면 기존 인덱스 삭제 후 재생성."""
    from app.opensearch.index import ensure_books_index
...
def restore_books(input_path: Path, clear: bool) -> int:
    """백업 파일에서 books 인덱스를 복원합니다.
    ...
    _ensure_books_index(os_client, clear)
...
def main() -> None:
    """스크립트 진입점."""
    parser = argparse.ArgumentParser(description="books OpenSearch 인덱스 복원 (재임베딩 없이)")
    ...
    count = restore_books(Path(args.input), args.clear)
